...
describe("generate excel file", () => {
  ...
  const reportServiceProto = Object.getPrototypeOf(reportService);
    it("should return the sprint statistics map when given sprint statistics", () => {
      const iterationDataMap = reportServiceProto.getSprintStatisticsMap(sprintStatistics);
    ...
    it("should return the empty sprint statistics map when given empty sprint statistics", () => {
      ...
      const emptyIterationDataMap = reportServiceProto.getSprintStatisticsMap(emptySprintStatistics);
      const expected = new Map();
  ...
  it("should generate the file when given time stamp", async () => {
    ...
    fs.rm("xlsx/exportSprintExcel-11.xlsx",()=>{return true;});
  });
});
