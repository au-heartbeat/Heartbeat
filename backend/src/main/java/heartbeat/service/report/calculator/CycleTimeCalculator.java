...
public class CycleTimeCalculator {
	...
	public CycleTime calculateCycleTime(CardCollection cardCollection,
		...
		CycleTimeResult cycleTimeResult = calculateAverageTimeAndTotalTime(aggregatedMap, cardCollection, selectedStepsMap);
	...
	private CycleTimeResult calculateAverageTimeAndTotalTime(Map<String, Double> aggregatedMap,
		...
		for (Map.Entry<String, Double> entry : aggregatedMap.entrySet()) {
			...
			if (List.of(CardStepsEnum.ANALYSE, CardStepsEnum.TODO)
				.contains(CardStepsEnum.fromValue(key)) || realDoneKeys.contains(key.toUpperCase())) {
				continue;
			}
