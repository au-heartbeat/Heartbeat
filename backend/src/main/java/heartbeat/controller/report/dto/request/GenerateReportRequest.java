...
public class GenerateReportRequest {
	...
	public List<String> getPipelineMetrics() {
		return this.metrics.stream().map(String::toLowerCase).filter(MetricsUtil.BUILDKITE_METRICS::contains).toList();
	}
	...
	public List<String> getSourceControlMetrics() {
		return this.metrics.stream().map(String::toLowerCase).filter(MetricsUtil.CODEBASE_METRICS::contains).toList();
	}

	@JsonIgnore
	public List<String> getBoardMetrics() {
		return this.metrics.stream().map(String::toLowerCase).filter(MetricsUtil.KANBAN_METRICS::contains).toList();
	}
