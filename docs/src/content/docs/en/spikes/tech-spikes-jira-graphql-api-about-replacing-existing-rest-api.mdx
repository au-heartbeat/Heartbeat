...
The existing Jira API are all RESTFUL API, and the RESTFUL API has the problem of over-fetching or multiple requests.
...
### One. For heartbeat's API `/boards/{boardType}/verify`
...
### Two. For heartbeat's API `/boards /{boardType}` and `/boards /{boardType}/info`
