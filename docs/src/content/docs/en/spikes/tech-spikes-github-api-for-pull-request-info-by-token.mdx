...
frontend -> backend: call POST /source/{sourceType}/repos/branches/verify to verify branch
backend -> githubClient: verify branch
githubClient -> backend: return ok
backend --> frontend: return verify successfully
...
#### 1. When the backend returns the organization to the frontend, should the avatar be returned for display on the frontend?
  - Advantage
    - user-friendly
  - Disadvantage
    - more efforts

#### 2. When the user selects a branch, does the operation of getting crews need to wait until the branch is verified?
  - Advantage
    - Asynchronous execution, no need to wait for branch verification to complete before getting crews, faster
  - Disadvantage
    - If the verification fails and user selects another branch, the crews that user selects may be changed.
