...
export const DoraDetail = withGoBack(({ data }: Property) => {
  ...
  return (
    <>
      ...
      {showSection(METRICS_TITLE.DEV_CHANGE_FAILURE_RATE, mappedData.changeFailureRateList)}
      {showSection(METRICS_TITLE.DEV_MEAN_TIME_TO_RECOVERY, mappedData.meanTimeToRecoveryList)}
    </>
  );
});
